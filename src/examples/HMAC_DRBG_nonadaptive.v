...
Theorem Gi_normal_prf_eq_compspec_i0 : forall (k1 k2 v : Bvector eta) (calls : nat) l,
    (* i <= length l -> *)
   comp_spec
     (fun (x : list (list (Bvector eta)) * (nat * KV))
        (y : list (list (Bvector eta)) * (nat * KV) * unit) =>
     ...
     ((oracleCompMap_inner
        ...
        (f_oracle f eqdbv k1) tt).
Proof.
  intros. revert k1 k2 v calls. unfold oracleMap. 
  induction l as [ | x xs]; intros; simplify.
  * fcf_spec_ret.
  * unfold Oi_prg. simpl. unfold Oi_oc'.
    simplify.
    (* oh yeah, it differs between GenUpdate_noV and GenUpdate (depending on where calls start from)... but does the output differ? 
and if calls is fixed at 0, then the induction hypothesis doesn't work? *)
(* note that even in oracleMap, the x is "trapped inside"? will induction work for these computations? *)
    apply IHxs.
    (* unfold oracleMap. *)
    (* unfold oracleCompMap_inner. *)
    (* unfold Oi_prg. *)
